...
def smpl_to_coco_body_subset(smpl_joints_2d):
    ...
    coco_body[0] = smpl_joints_2d[15]
    coco_body[5] = smpl_joints_2d[17]
    coco_body[6] = smpl_joints_2d[16]
    coco_body[7] = smpl_joints_2d[19]
    coco_body[8] = smpl_joints_2d[18]
    coco_body[9] = smpl_joints_2d[21]
    coco_body[10] = smpl_joints_2d[20]
    coco_body[11] = smpl_joints_2d[2]
    coco_body[12] = smpl_joints_2d[1]
    coco_body[13] = smpl_joints_2d[5]
    coco_body[14] = smpl_joints_2d[4]
    coco_body[15] = smpl_joints_2d[8]
    coco_body[16] = smpl_joints_2d[7]

    coco_body[1:5] = smpl_joints_2d[15]
...
def process_sequence(seq_name, split, pose_detector, rest_pose_data):
    ...
    print(f"  Total frames: {total_frames}, Matched: {matched_frames} ({matched_frames/total_frames*100:.1f}%)")
    if SAVE_VISUALIZATIONS:
        print(f"  Visualizations: {visualized_frames} frames")

    for actor_idx in range(num_actors):
        match_rate = matched_counts[actor_idx] / total_frames if total_frames > 0 else 0
        print(f"  Actor {actor_idx}: {matched_counts[actor_idx]}/{total_frames} ({match_rate:.1%})")

def main():
    print("3DPW preprocessing")
    ...
    print(f"Initialized pose detector on {device}")
    ...
    print(f"Results: {DETECTIONS_DIR}")
    if SAVE_VISUALIZATIONS:
