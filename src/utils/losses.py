...
def bone_length_consistency_loss(predicted_poses, target_poses=None):
    ...
    for parent, child in _edges:
        ...
        target_length = bone_matrix[parent, child]

        error = (pred_length - target_length) ** 2
        bone_errors.append(error)

    bone_errors = torch.stack(bone_errors, dim=1)
...
def projection_loss_with_visibility(pred_3d_centered, gt_2d, camera_params, root_translation,
    ...
    width = resolution[:, 0].unsqueeze(1).unsqueeze(2)
    height = resolution[:, 1].unsqueeze(1).unsqueeze(2)
